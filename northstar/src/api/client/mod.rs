...
use super::{
    codec,
    model::{
        Connect, ConnectNack, Container, ContainerData, ContainerStats, Message, MountResult,
...
impl<'a, T: AsyncRead + AsyncWrite + Unpin> Client<T> {
    ...
    /// # }
    /// ```
    pub async fn containers(&mut self) -> Result<Vec<ContainerData>, error::RequestError> {
    ...
    /// # }
    /// ```
    pub async fn container_stats<C>(
        &mut self,
        container: C,
    ) -> Result<ContainerStats, error::RequestError>
    where
        C: TryInto<Container>,
        C::Error: std::error::Error + Send + Sync + 'static,
    {
